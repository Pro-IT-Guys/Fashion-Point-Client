...
export default function Terms() {
  ...
  const onSubmit = async data => {
    console.log(content)
    const res = await fetch(`${BASE_URL}/terms-condition`, {
      method: 'POST',
      headers: {
    ...
    if (result.success) {
      ...
      Swal.fire({
        icon: 'success',
        title: 'Success',
        text: 'Terms & Conditions added successfully',
      })
    }
  }
  ...
  return (
    <DashboardLayout>
      <Container maxWidth="lg">
        <h1 className="text-2xl font-semibold mb-8">Add Terms & Conditions</h1>
